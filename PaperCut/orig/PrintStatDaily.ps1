####################################################################################
# Автор: Сулев Илья						  		   #
# Дата: 18-01-2024 Версия: 1.2					  		   #
# Назначение: Скрипт ежедневного получения статистики печати с принтеров 	   #
####################################################################################
...
$CSV1 = "C$\Program Files (x86)\PaperCut Print Logger\logs\csv\daily\papercut-print-log-$(get-date -f yyyy-MM-dd).csv"
$CSV2 = "C$\Program Files\PaperCut Print Logger\logs\csv\daily\papercut-print-log-$(get-date -f yyyy-MM-dd).csv"

#Шаблон таблицы
$Template = "\\omzglobal.com\uxm\Exchange\PrintStat\PSTools\Template.csv"

#Папка с результатами
$Path = "\\omzglobal.com\uxm\Exchange\PrintStat\Logs\Daily"
...
if (!(Test-Path -Path \\omzglobal.com\uxm\Exchange\PrintStat\Logs\Daily\PrintLog-$(get-date -f dd-MM-yyyy).csv)) {
Get-Content $Template -Encoding UTF8 | Out-File $Path\PrintLog-$(get-date -f dd-MM-yyyy).csv -Force -Encoding UTF8
Write-Host "[ИНФО] Создан новый журнал в директории [\\$Path\PrintLog-$(get-date -f dd-MM-yyyy).csv]" -foregroundColor Cyan
}
...
#Проверка на наличие отключенных компьютеров. Если включен проверяем наличие каталога службы, если его нет то устанавливаем службу
		...
		Write-Host "$PrintLog"
		$PrintLog | Out-File $Path\PrintLog-$(get-date -f dd-MM-yyyy).csv -Encoding UTF8 -Append -Force
}} else {
		$Text = "{0} : [ОШИБКА!] Компьютер: [{1}] не отвечает на сетевые запросы" -f (Get-Date).ToString(), $CompName
        	$Log.AppendLine($Text) | Out-Null
       		Write-Host "[ОШИБКА] $CompName не отвечает..." -foregroundColor Yellow  
}} Catch {
    ...
    $Log.AppendLine($Text) | Out-Null
		...
		$Text = "{0} : [ИНФО!] Компьютер: [{1}] анализ журналов печати завершен" -f (Get-Date).ToString(), $CompName
        	$Log.AppendLine($Text) | Out-Null
		$Jobs = Get-Content -Path $Path\PrintLog-$(get-date -f dd-MM-yyyy).csv -ErrorAction SilentlyContinue | Sort-Object -Unique -Descending
		$Jobs | Out-File $Path\PrintLog-$(get-date -f dd-MM-yyyy).csv -Encoding UTF8 -Force
...
$SearchRootSRV="OU=Terminal,OU=uxm,OU=servers-ekb-uhm,OU=Servers,DC=omzglobal,DC=com"
#Для тестов
...
#Проверка на наличие отключенных серверов. Если включен проверяем наличие каталога службы, если его нет то устанавливаем службу
		...
		if (!(Test-Path -Path "\\$ServerName\C$\Program Files (x86)\PaperCut Print Logger") -And (!(Test-Path -Path "\\$ServerName\C$\Program Files\PaperCut Print Logger"))) {
		...
		\\omzglobal.com\uxm\Exchange\PrintStat\PSTools\psexec.exe -accepteula -s -h \\$ServerName -c -v \\omzglobal.com\uxm\Exchange\PrintStat\PSTools\papercut-print-logger.exe /NOICONS /VERYSILENT /SUPPRESSMSGBOXES /TYPE=secondary_print
	   	$Text = "{0} : [ИНФО!] Сервер: [{1}] служба печати успешно установлена" -f (Get-Date).ToString(), $ServerName
		...
		$Text = "{0} : [ОШИБКА!] Сервер: [{1}] не отвечает на сетевые запросы" -f (Get-Date).ToString(), $ServerName
        	$Log2.AppendLine($Text) | Out-Null
       		Write-Host "[ОШИБКА] $ServerName не отвечает..." -foregroundColor Yellow
}} Catch {
    $ErrorMessage = $_.Exception.Message
...
#Чистим последние дубликаты и сохраняем результат
$Jobs = Get-Content -Path $Path\PrintLog-$(get-date -f dd-MM-yyyy).csv -ErrorAction SilentlyContinue | Sort-Object -Unique -Descending
$Jobs | Out-File $Path\PrintLog-$(get-date -f dd-MM-yyyy).csv -Encoding UTF8 -Force
Write-Host "[ИНФО] Все данные успешно сохранены в журнал" -foregroundColor Cyan
