...
Invoke-Command -ComputerName $who -ScriptBlock {
  ...
  $Task.Triggers[0].EndBoundary = (Get-Date).AddMinutes(100).ToString('s')
  Register-ScheduledTask -TaskName "Restart!" -TaskPath uxm -InputObject $Task -User "System" -Force | Out-Null
}
