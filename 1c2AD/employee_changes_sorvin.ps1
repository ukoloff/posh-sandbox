...
Foreach ($User in $Users) {
	Try {
		...
		$Log.AppendLine($Text) | Out-File C:\Scripts\employee_changes_log.txt -Append    
	} Catch {
		...
		$Log.AppendLine($StatusText) | Out-File C:\Scripts\employee_changes_log.txt -Append
    } Finally {
        #Write-Host $Log -foregroundColor Green
		$Log.ToString() | Out-File C:\Scripts\employee_changes_log.txt -Append
    }
}
